...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function populateTaskInstanceProperties(node) {
    ...
    for (j = 0; j < node.instances.length; j += 1) {
      ...
      if (dataInstance && dataInstance.execution_date) {
        ...
        if (row && row.length) {
          [taskInstance.state, taskInstance.try_number, taskInstance.start_ts, taskInstance.duration] = row; // compute start_date and end_date if applicable
  ...
  function update(source, showTransition = true) {
    ...
    nodeEnter.append('g').attr('class', 'stateboxes').attr('transform', d => `translate(${squareX - d.y},0)`).selectAll('rect').data(d => d.instances).enter().append('rect').on('click', d => {
    ...
    }).attr('data-toggle', 'tooltip').attr('rx', d => isDagRun(d) ? '5' : '1').attr('ry', d => isDagRun(d) ? '5' : '1').style('shape-rendering', d => isDagRun(d) ? 'auto' : 'crispEdges').style('stroke-width', d => isDagRun(d) ? '2' : '1').style('stroke-opacity', d => {
      ...
      taskTip.direction('n');
      taskTip.show(tt, this);
      d3.select(this).transition().duration(duration).style('stroke-width', dd => dd.external_trigger ? '1' : '3');
    }).on('mouseout', function (d) {
      taskTip.hide(d);
      d3.select(this).transition().duration(duration).style('stroke-width', dd => isDagRun(dd) ? '2' : '1');
